...
def event_relation(tableOfEvents,numberOfNodes):
	...
	for node in range(0,numberOfNodes):
		if(node!=0):
			tableOfEvents[node][1]=int(input("[DATA] Enter the preceding node for %d node: "%(node)))
...
def abs_events(tableOfEvents,numberOfNodes):
	for node in range(1,numberOfNodes):
		node=int(tableOfEvents[node][1])
		tableOfEvents[node][3]=tableOfEvents[node][2]+tableOfEvents[node][3]
...
def graph_builder(tableOfEvents,processGraph,numberOfNodes):
	...
	for node in range(1,len(tableOfEvents)):
		processGraph.edge(str(int(tableOfEvents[node][1])),str(int(node)),label=("Forward Pass Time: "+str(int(tableOfEvents[node][3]))))
